...
impl<'data> TxRing<'data, NotRunning> {
    ...
    pub fn start(mut self, eth_dma: &ETHERNET_DMA) -> TxRing<'data, Running> {
        ...
        // Write tail pointer register
        #[cfg(feature = "stm32h7xx-hal")]
        eth_dma.dmactx_dtpr.write(|w| {
            w.tdt()
                .variant(self.ring.last_descriptor() as *const _ as u32)
        });
        ...
        eth_dma
            .dmactx_dlar
            .write(|w| w.tdesla().variant(ring_ptr as u32));
...
impl<'data> TxRing<'data, Running> {
    ...
    pub fn demand_poll(&self, eth_dma: &ETHERNET_DMA) {
        #[cfg(feature = "stm32h7xx-hal")]
        {
            ...
            // current value.
            let value = eth_dma.dmactx_dtpr.read().tdt().bits();

            eth_dma.dmactx_dtpr.write(|w| w.tdt().variant(value));
        }
